...
- **Test suite:** 865 tests across 16 files in `tests/` (last refresh 2026-04-21). Sequential run ~400 s; parallel run (`pytest -n auto`) ~125 s on 8 cores (~3× speedup). Biology subset (`test_dna_sanity.py`) < 1 s. `test_invariants_hypothesis.py` adds property-based fuzzing on top of hand-written regression tests.
...
| Helper | Line | Purpose |
|---|---:|---|
| `_safe_save_json` / `_safe_load_json` / `_extract_entries` | 282 / 362 / 228 | Atomic JSON I/O with `.bak` recovery and schema-envelope format. All four libraries go through these. |
| `_atomic_write_text` | 251 | Module-level atomic write helper (tempfile + fsync + os.replace + cleanup). `_do_save`, `_do_autosave`, `_export_genbank_to_path`, `_export_fasta_to_path` all route through this — a crash mid-write leaves the previous file intact. |
...
| `test_data_safety.py` | 45 | Sacred invariant #7 (atomic saves, `.bak` recovery); **schema-envelope round-trip + legacy bare-list back-compat + future-version warning + shrink-guard counting both formats**; `features.json` redirected by `_protect_user_data`; `_protect_user_data` fixture confirmation; **`_atomic_write_text` helper** (new-file / overwrite / tempfile cleanup / error preserves previous file / nested dir creation / unicode) + **`_do_save` atomicity regression guard** (simulated replace failure must leave original .gb intact) |
...
2. **Run `python3 -m pytest -n auto -q`** before and after any change. 865 tests, ~125 s on 8 cores (or ~400 s serial). Biology subset (`tests/test_dna_sanity.py`) runs in < 1 s for a fast inner loop.
