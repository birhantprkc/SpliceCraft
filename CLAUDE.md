...
- `splicecraft_fileio` (Phase D, L2 — single-file sequence-format I/O: FASTA/AB1/FASTQ/GFF3 ingest (path→record) + GenBank/GFF/FASTA/EMBL export, with codec helpers. Lazy Biopython; imports record L1 + persistence/logging L0. **CLEAN single-file subset ONLY** — the `.dna` CommercialSaaS reader/writer, the `_save`/`_load_dna_original` blob store, network `fetch_genbank`, and the zip/Plasmidsaurus member handling STAY hub-side (separate concerns / data-safety) for a later step. GOTCHA: the moved format caps `_FASTQ_MAX_READS`/`_BULK_IMPORT_MAX_BYTES` are read in this sibling's namespace — tests gating the parsers patch `splicecraft_fileio.X`, not `sc.X`)
